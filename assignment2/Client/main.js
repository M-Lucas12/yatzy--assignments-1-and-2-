...
async function rollDice() {
    ...
    try {
        // ✅ 1) Start rolling animation ONLY for non-held dice (Lab 8 style)
        diceButtons.forEach(btn => {
            const held = btn.getAttribute("aria-pressed") === "true";
            if (!held) btn.classList.add("rolling");
        });

        // ✅ 2) Let the shake play for a moment
        await new Promise(res => setTimeout(res, 700));

        // ✅ 3) Now call server to roll
        const data = await api(`/api/games/${currentGameId}/roll`, "POST");

        // ✅ 4) Stop shaking
        diceButtons.forEach(btn => btn.classList.remove("rolling"));

        // ✅ 5) Render final dice results
        renderGame(data.game);
        setStatus(data.message);
    } catch (err) {
        console.error(err);
